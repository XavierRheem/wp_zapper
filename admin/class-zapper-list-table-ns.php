...
class Zapper_List_Table_NS extends Libraries\WP_List_Table  {
    ...
    public function get_columns() {
        ...
        $table_columns = array(
            ...
            'wpmmv'        =>    __( 'User Group', $this->plugin_text_domain ),            
            'wpm2mv'    => __( 'SMS Number', 'column name', $this->plugin_text_domain ),
    ...
    public function fetch_table_data() {
        ...
        $wpdb_table = $wpdb->prefix . 'usermeta';        

        $orderby = ( isset( $_GET['orderby'] ) ) ? esc_sql( $_GET['orderby'] ) : 'wpmmv';
        $order = ( isset( $_GET['order'] ) ) ? esc_sql( $_GET['order'] ) : 'ASC';

        $user_query = 'SELECT DISTINCT wpu.ID, wpu.display_name, wpu.user_login, wpm2.meta_value wpm2mv, wpm.meta_value wpmmv FROM wp_users wpu LEFT JOIN wp_usermeta wpm ON wpm.user_id = wpu.ID LEFT JOIN wp_usermeta wpm2 ON wpm2.user_id = wpu.ID WHERE wpm.meta_key LIKE "sms_usergroup" AND wpm2.meta_key LIKE "sms_number" ORDER BY '.$orderby.' '.$order;  
    ...
    protected function column_user_login( $item ) {
        ...
        $actions['usergroup_view'] = '<a href="' . $usergroup_view_link . '">' . __( 'View', $this->plugin_text_domain ) . '</a>';        
        ...
        $actions['usergroup_edit'] = '<a href="' . $usergroup_edit_link . '">' . __( 'Edit', $this->plugin_text_domain ) . '</a>';            
    ...
    public function get_bulk_actions() {
         ...
         */
         $actions = array(
             'selected_bulk_edit' => 'Selected Users Edit',
             'selected-bulk-email' => 'Selected Users Email',
             'selected-bulk-sms' => 'Selected Users SMS',
             'group-retired-bulk-sms' => 'Group SMS Retired',
             'group-active-bulk-sms' => 'Group SMS Active',
             'group-all-bulk-sms' => 'Group SMS All'
         );
